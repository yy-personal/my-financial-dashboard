...
const EditParameters = () => {
	...
	// Use useRef to keep track of next id for yearly expenses
	const nextYearlyExpenseId = useRef(1);
	...
	const initializeFormData = () => {
		...
		// Check if there are yearly expenses in the context
		const yearlyExpenses = financialData.yearlyExpenses || [];

		// Find the largest ID for yearly expenses to update the ref
		if (yearlyExpenses.length > 0) {
			const maxExpenseId = Math.max(
				...yearlyExpenses.map((expense) => expense.id)
			);
			nextYearlyExpenseId.current = maxExpenseId + 1;
		}
		...
		yearlyExpenses: yearlyExpenses.map((expense) => ({
			...expense,
			amount: String(expense.amount),
		})),
	...
	// Handle yearly expense field changes
	const handleYearlyExpenseChange = (index, field, value) => {
		setFormData((prevData) => {
			const newData = { ...prevData };
			const updatedExpenses = [...newData.yearlyExpenses];
			updatedExpenses[index] = {
				...updatedExpenses[index],
				[field]: value,
			};
			newData.yearlyExpenses = updatedExpenses;
			return newData;
		});
	};

	// Handle changes to new yearly expense form
	const handleNewYearlyExpenseChange = (field, value) => {
		setFormData((prevData) => {
			const newData = { ...prevData };
			newData.newYearlyExpense = {
				...newData.newYearlyExpense,
				[field]: value,
			};
			return newData;
	...
	// Handle adding a new yearly expense
	const handleAddYearlyExpense = () => {
		const { name, amount, month, startYear, endYear, isRecurring, description } = formData.newYearlyExpense;
		...
		if (name && amount) {
			...
			setFormData((prevData) => {
				const newData = { ...prevData };
				newData.yearlyExpenses = [...newData.yearlyExpenses, newExpense];
	...
	// Handle removing a yearly expense
	const handleRemoveYearlyExpense = (id) => {
		setFormData((prevData) => {
			const newData = { ...prevData };
			newData.yearlyExpenses = newData.yearlyExpenses.filter(
	...
	// Process yearly expenses - convert strings to proper types
	const processedYearlyExpenses = formData.yearlyExpenses.map((expense) => ({
		...expense,
		amount: parseFloat(expense.amount) || 0,
		month: parseInt(expense.month) || 1,
		startYear: parseInt(expense.startYear) || new Date().getFullYear(),
		endYear: expense.endYear ? parseInt(expense.endYear) : null,
	}));
		...
		yearlyExpenses: processedYearlyExpenses,
		});
